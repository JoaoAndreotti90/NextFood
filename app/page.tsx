    ...
    <div className="min-h-screen bg-gray-50 pb-24">
        ...
        <div className="max-w-7xl mx-auto px-6 py-6 flex items-center justify-between gap-6">
          <Link href="/" className="flex flex-col group">
            <h1 className="text-3xl font-extrabold text-white tracking-tight flex items-center gap-2">
              NextFood{" "}
              <Utensils className="opacity-80 group-hover:rotate-12 transition-transform" />
            </h1>
          </Link>
      ...
      <main className="max-w-7xl mx-auto px-4 py-10 space-y-12">
        <section>
          ...
          <div className="flex gap-4 overflow-x-auto p-4 scrollbar-hide -mx-4 px-8 sm:mx-0 sm:px-4">
            <Link
              href="/"
              className={`flex flex-col items-center gap-3 min-w-[85px] cursor-pointer group ${
                !categoryId ? "opacity-100" : "opacity-60 hover:opacity-100"
              }`}
            >
              <div
                className={`w-20 h-20 rounded-[1.5rem] flex items-center justify-center shadow-sm transition-all ${
                  !categoryId
                    ? "bg-orange-600 text-white shadow-orange-200 shadow-lg scale-105"
                    : "bg-white text-gray-400 group-hover:bg-orange-50 group-hover:text-orange-500"
                }`}
              >
                <Utensils size={28} />
              </div>
              <span
                className={`text-xs font-bold ${
                  !categoryId ? "text-orange-600" : "text-gray-500"
                }`}
              >
                Todas
              </span>
            ...
            {categories.map((category) => (
              ...
              >
                <div
                  className={`w-20 h-20 rounded-[1.5rem] overflow-hidden shadow-sm relative transition-all group-hover:scale-105 group-hover:shadow-md 
                  ${
                    categoryId === category.id
                      ? "ring-4 ring-orange-600 ring-offset-2 scale-105 shadow-lg"
                      : "bg-white"
                  }`}
                >
                  <Image
        ...
        <section>
          ...
          {restaurants.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 bg-white rounded-[2.5rem] border border-gray-100 shadow-sm mx-2">
              ...
              <Link
                href="/"
                className="text-orange-600 text-sm font-bold hover:underline mt-2"
              >
                Limpar filtros
              </Link>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 px-2">
              {restaurants.map((restaurant) => (
                <Link
                  ...
                  className="group bg-white rounded-[2rem] overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-gray-100"
                >
                  ...
                  <div className="p-5">
                    <div className="flex justify-between items-start mb-2">
                      <h3 className="font-extrabold text-gray-800 text-lg group-hover:text-orange-600 transition-colors line-clamp-1">
                        {restaurant.name}
                      </h3>
                    </div>
